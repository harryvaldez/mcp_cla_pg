...
def create_connection_pool():
    ...
    try:
        pool = ConnectionPool(
            ...
            max_lifetime=600,
            timeout=10,
            check=psycopg.Connection.is_ready
        )
        return pool
...
def main():
    ...
    test_configs = [
        ...
        {"threads": 5, "iterations": 3},
        {"threads": 10, "iterations": 2}
    ]
    ...
    for config in test_configs:
        ...
        results = run_tool_stress_test(config["threads"], config["iterations"])
        analysis = analyze_stress_results(results)
